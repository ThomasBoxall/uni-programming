...
class TwoThreeTree:
  ...
  def _insert(self, node, key):
    if node.hasKey(key): # duplicate key
      ...
      child = node.getChild(key) # get reference to the child/subtree for further operations
  ...
  def bft(self):
    ...
    while queue:
      ...
      if current_node:
        result.append([current_node.key1, current_node.key2] if current_node.key2 else [current_node.key1])
